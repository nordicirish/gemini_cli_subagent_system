...
- **Friction Aware Entry Logic:**
  ...
  - **Logic:** IF Action == ENTRY AND (Abs(Next_Resistance - Current_Price) / Current_Price < Gemini_Gem_Working_Data_Store.system_thresholds.GLOBAL_ALPHA_FRICTION_HURDLE) THEN Confidence_Score = 0.4 (Non-Actionable) AND TAG 'Low_Alpha_Headroom'. Bind `Next_Resistance` directly to `fib_forecast.next_resistance` from the quantitative state (ENH_254).
