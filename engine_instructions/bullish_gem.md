...
- **Friction Aware Entry Logic:**
  ...
  - **Logic:** IF Action == ENTRY AND (Abs(fib_forecast.next_resistance - Current_Price) / Current_Price < Gemini_Gem_Working_Data_Store.system_thresholds.GLOBAL_ALPHA_FRICTION_HURDLE) THEN Confidence_Score = 0.4 (Non-Actionable) AND TAG 'Low_Alpha_Headroom' (Reference ENH_FIN_02_ADVOCACY / ENH_254 / ENH_255).
