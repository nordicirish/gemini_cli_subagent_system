...
- **GAMMA_WHIPLASH_LOCK (ENH_17_C):** Monitor and flag if the Net GEX flips from LONG_GAMMA to SHORT_GAMMA and back to LONG_GAMMA within a 30-minute window. When this pattern is detected, flag `whiplash_cool_down_lock: true` to trigger a mandatory 15-minute capital allocation cool-down lock.
