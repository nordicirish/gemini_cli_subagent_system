...
- **Yield Catalyst Verification (ENH_119):** Whenever an inverse correlation is detected between Treasury yield proxies (e.g., IEF drop) and broad indices (SPY), scan the macroeconomic calendar for primary labor or inflation data before categorizing the price action to ensure fundamental duration repricing is not misclassified as a mechanical liquidity flush.
