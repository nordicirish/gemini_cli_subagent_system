...
subagent_instructions = [
    "macro_sentinel.md", "research.md", "sentiment_engine.md",
    "structural_engine.md", "context_engine.md", "technical_validator.md",
    "bullish_gem.md", "red_team_gem.md", "neutral_gem.md", "terminal.md",
...
structural_engine = framework.create_agent_tool(
    ...
    file_path="structural_engine.md", 
    mode="GEMMA",
...
technical_validator = framework.create_agent_tool(
    ...
    file_path="technical_validator.md",
    mode="GEMMA",
...
post_trade_review = framework.create_agent_tool(
    ...
    file_path="post_trade_review.md",
    mode="FAST",
...
def chat_endpoint(req: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            if not trade_lessons and os.path.exists("trade_lessons.json"):
                try:
        ...
        # Intercept and log decisions from the full chat response
        try:
...
def save_decision_log(req: DecisionLogRequest):
    ...
    try:
        ...
        lessons_file = "decision_log.json"
        existing = []
...
def get_scout_categories():
    try:
        if os.path.exists("ssot.json"):
            with open("ssot.json", "r") as f:
                data = json.load(f)
                return data.get("scout_categories", [])
    except: pass
    return []

@app.post("/api/scout_categories")
def save_scout_categories(categories: List[str]):
    try:
        if os.path.exists("ssot.json"):
            with open("ssot.json", "r") as f:
                data = json.load(f)
